...
const DEFAULT_PATTERNS = ["\\b(?:trellis|mx)-[0-9a-f]+\\b", "#\\d+\\b", "https?://\\S+"];
...
describe("loadBudget", () => {
	test("parses tracker patterns into RegExp instances", () => {
		...
		try {
			...
			expect(trackerRegexes[0]?.test("// TODO(trellis-abc1): later")).toBe(true);
...
describe("scan — synthetic pass", () => {
	test("returns no untracked markers when every marker has a tracker", () => {
		...
		try {
			writeFileTree(root, {
				"src/a.ts": "// TODO(trellis-7f2b): later\nexport const a = 1;\n",
	...
	test("placeholder strings like trellis-XXXX are not flagged as markers", () => {
		const { root, cleanup } = makeFixture();
		try {
			writeFileTree(root, {
				"src/a.ts":
					"// example placeholder: trellis-XXXX or mx-XXXX (no real marker)\nexport const a = 1;\n",
			});
			const budgetPath = join(root, "budget.json");
			writeBudget(budgetPath, DEFAULT_PATTERNS);
			const result = scan({
...
describe("co-located budget JSON", () => {
	test("scripts/debt-markers-budget.json is well-formed", () => {
		const raw = JSON.parse(
			readFileSync(resolve(import.meta.dir, "debt-markers-budget.json"), "utf8"),
