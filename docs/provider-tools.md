...
For knip 6.16.1: **linux-x64-gnu** is `tested` (the adapter's conformance
and failure-regression suites + the provider smoke). No other platform is
declared — the pinned distribution is host-independent JavaScript, but
trellis claims only what it exercised.
