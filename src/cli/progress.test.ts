...
/**
 * The CLI progress reporter. A non-verbose TTY rewrites a single status line in
 * place (`\r`) tracking the current phase + progress; `--verbose` switches to a
 * durable line-per-event log (any stream) with per-detector detail; `--quiet`
 * (and a non-verbose non-TTY) stays silent. A captured `write` sink keeps these
 * tests stream-free.
 */
...
describe("createProgressReporter", () => {
	...
	test("rewrites one in-place status line tracking phase + progress on a TTY", () => {
		...
		reporter?.onProgress({ type: "phase", phase: "detectors" });
		reporter?.onProgress({ type: "detector", id: "lint_config", index: 0, total: 70 });
		...
		expect(last).toContain("running detectors (1/70)");
	});
	...
	test("--verbose logs durable per-detector lines and finish does not clear", () => {
		...
		reporter?.onProgress({ type: "detector", id: "L2-foo", index: 0, total: 70 });
		reporter?.finish();
		const text = lines.join("");
		expect(text).toContain("[1/70] L2-foo");
	...
	test("--verbose forces rendering even on a non-TTY run and shows phase lines", () => {
		...
		reporter?.onProgress({ type: "phase", phase: "detectors" });
		expect(lines.join("")).toContain("running detectors");
	});
});
